...
parser.add_argument('--target', default='D', help='target domain strins : GSMIC, it can be multi targets')
parser.add_argument('--normal-eval', type=bool, default=True, help='normally eval the network')
parser.add_argument('--target-eval', type=bool, default=False, help='target-specific eval the network')
...
def test_one_run(framework, run_name, targets, batches=[16], normal_eval=True, target_specific_eval=True, **kwargs):
    ...
    framework.load('best_dada_seg', strict=False)
...
def do_lots_of_exp_tests(names, targets, batch_sizes, **kwargs):
