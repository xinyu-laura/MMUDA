...
class WarmUpLRScheduler(object):
    ...
    def step(self, epoch, iter):
        ...
        if self.warmup_epochs > 0 and T > 0 and epoch < self.warmup_epochs:
            ...
            lr_list = [lr * 1.0 * T / (self.warmup_epochs*self.iteration_per_epoch) for lr in self.base_lrs]
            # lr_list = [lr * (1-(1 - 1.0 * T / self.warmup_epochs*self.iteration_per_epoch)*(1 - self.warmup_ratio)) for lr in self.base_lrs]
